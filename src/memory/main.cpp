...
int main(){
    ...
    // std::bitset<8> test = 0b11001000;
    // std::cout << "test: " << test << std::endl;
    for (auto i = Iram->getAddrRange().first; i < Iram->getAddrRange().second; i+=4){
        Iram->writeWordAtAddr(i, 1430400000);
            std::cout << Iram->readWordAtAddr(i)/1 << std::endl;
    }
...
//      CPU * dev0 = new CPU(sys, "cpu0", 0, 0x93, 0x200, 1023);
//      CPU * dev1 = new CPU(sys, "cpu1", 0, 0x93, 0x200, 1023);
//      // stores a 1KB array
...
//      Iram->getPort()->bind(bus->getUnboundMemSidePort());
//      dev0->getPort1()->bind(bus->getUnboundCPUSidePort());
//      dev1->getPort1()->bind(bus->getUnboundCPUSidePort());
//
...
//           // float r = static_cast <float> (rand()) / static_cast <float> (RAND_MAX);
//           // uint32_t val = *(uint32_t *)(&r);
//           Iram->writeWordAtAddr(i, 400000);
//           std::cout << Iram->readWordAtAddr(0) << std::endl;
//      }
//
