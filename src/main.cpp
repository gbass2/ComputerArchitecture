...
int main(){
    ...
    auto cpu0 = std::make_shared<CPU>(sys, "cpu0", 0, 1023);
    // auto cpu1 = std::make_shared<CPU>(sys, "cpu1", 0, 1023);

    RunSim *sim = new RunSim(sys, cpu0);

    DRAM * Iram = new DRAM(sys, AddrRange(0,0x94), 20);          // 20 = response time
    DRAM * ram = new DRAM(sys, AddrRange(0x200, 0x13FF), 20);    // 20 = response time
    ...
    Iram->getPort()->bind(bus->getUnboundCPUSidePort());    // Instruction RAM
